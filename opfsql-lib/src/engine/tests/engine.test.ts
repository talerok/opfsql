...
describe("initialization", () => {
  ...
  it("Engine.create loads catalog from storage", async () => {
    ...
    s.execute("INSERT INTO t (id) VALUES (42)");
    s.close();
    e.close();

    engine = await Engine.create(getStorage(name));
    session = engine.createSession();
    const [result] = session.execute("SELECT * FROM t");
...
describe("autocommit", () => {
  it("SELECT does not persist writes", async () => {
    ...
    session.execute("INSERT INTO t (id, name) VALUES (1, 'a')");
...
describe("explicit transactions", () => {
  ...
  it("BEGIN + CREATE TABLE + ROLLBACK removes table from catalog", async () => {
    ...
    expect(() => session.execute("SELECT * FROM t")).toThrow();
  });
  ...
  it("ROLLBACK without BEGIN is a no-op", async () => {
    await createEngine();
    const [result] = session.execute("ROLLBACK");
    expect(result.type).toBe("ok");
  });
  ...
  it("nested BEGIN throws EngineError", async () => {
    await createEngine();
    session.execute("BEGIN");
    expect(() => session.execute("BEGIN")).toThrow(EngineError);
  });
});
...
describe("multiple statements", () => {
  ...
  it("statement execution order is preserved", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, val TEXT)");
...
describe("column aliases", () => {
  it("AS alias appears as output row key", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, name TEXT)");
    session.execute("INSERT INTO t (id, name) VALUES (1, 'alice')");

    const [result] = session.execute("SELECT name AS username FROM t");
    expect(result.type).toBe("rows");
    expect(result.rows).toHaveLength(1);
  ...
  it("multiple aliases all appear in output", async () => {
    ...
    session.execute("INSERT INTO t (id, name, age) VALUES (1, 'alice', 30)");

    const [result] = session.execute("SELECT name AS n, age AS a FROM t");
  ...
  it("expression alias on computed column", async () => {
    ...
    session.execute("INSERT INTO t (id, price, qty) VALUES (1, 10.5, 3)");

    const [result] = session.execute("SELECT price * qty AS total FROM t");
...
describe("WHERE + column pruning", () => {
  it("SELECT subset with WHERE on non-selected column", async () => {
    ...
    session.execute("INSERT INTO t (id, name, age) VALUES (1, 'Alice', 30)");
    session.execute("INSERT INTO t (id, name, age) VALUES (2, 'Bob', 25)");
    session.execute("INSERT INTO t (id, name, age) VALUES (3, 'Charlie', 35)");

    const [result] = session.execute("SELECT name FROM t WHERE age > 28");
...
describe("DML without WHERE", () => {
  it("UPDATE all rows without WHERE", async () => {
    ...
    session.execute("UPDATE t SET age = 99");
    const [result] = session.execute("SELECT * FROM t");
  ...
  it("DELETE all rows without WHERE", async () => {
    ...
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, name TEXT)");
    session.execute("INSERT INTO t (id, name) VALUES (1, 'a')");
    session.execute("INSERT INTO t (id, name) VALUES (2, 'b')");
  ...
  it("UPDATE all rows — check actual values", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, age INTEGER)");
    session.execute("INSERT INTO t (id, age) VALUES (1, 10)");
    session.execute("INSERT INTO t (id, age) VALUES (2, 20)");

    const [updateResult] = session.execute("UPDATE t SET age = 99");
    const [after] = session.execute("SELECT * FROM t");
  ...
  it("UPDATE with WHERE on non-updated column", async () => {
    ...
    session.execute("UPDATE t SET name = 'X' WHERE age > 28");
    const [result] = session.execute("SELECT * FROM t");
...
describe("error handling", () => {
  it("error in middle of transaction aborts entire transaction", async () => {
    ...
    expect(() => session.execute("INSERT INTO t (id) VALUES (3)")).toThrow(
      /transaction is aborted/,
    );

    expect(() => session.execute("COMMIT")).toThrow(/ROLLBACK/);
  ...
  it("ROLLBACK after aborted transaction allows new work", async () => {
    ...
    session.execute("INSERT INTO t (id) VALUES (10)");
    const [result] = session.execute("SELECT * FROM t");
    expect(result.rows).toHaveLength(1);
...
describe("auto PK index", () => {
  it("PRIMARY KEY creates a unique index automatically", async () => {
    ...
    session.execute("INSERT INTO t VALUES (2, 'b')");
    session.execute("INSERT INTO t VALUES (3, 'c')");

    const [result] = session.execute("SELECT name FROM t WHERE id = 2");
    expect(result.rows).toHaveLength(1);
  ...
  it("PK index enforces uniqueness", async () => {
    ...
    expect(() => session.execute("INSERT INTO t VALUES (1, 'dup')")).toThrow();
  });
  ...
  it("PK index works with UPDATE and DELETE", async () => {
    ...
    session.execute("INSERT INTO t VALUES (1, 'reused')");
    const [result] = session.execute("SELECT name FROM t WHERE id = 1");
  ...
  it("PK index survives transaction rollback", async () => {
    ...
    session.execute("INSERT INTO t VALUES (2)");
    const [result2] = session.execute("SELECT * FROM t WHERE id = 2");
...
describe("ALTER TABLE ADD COLUMN defaults", () => {
  it("existing rows get DEFAULT value after ADD COLUMN", async () => {
    ...
    session.execute("ALTER TABLE t ADD COLUMN val INTEGER DEFAULT 99");

    const [result] = session.execute("SELECT id, val FROM t ORDER BY id");
  ...
  it("existing rows get NULL for ADD COLUMN without DEFAULT", async () => {
    ...
    session.execute("ALTER TABLE t ADD COLUMN val TEXT");

    const [result] = session.execute("SELECT id, val FROM t");
  ...
  it("new rows after ADD COLUMN get explicit value", async () => {
    ...
    session.execute("ALTER TABLE t ADD COLUMN val INTEGER DEFAULT 99");
    session.execute("INSERT INTO t (id, val) VALUES (2, 42)");
  ...
  it("UPDATE works on rows with default-filled column", async () => {
    ...
    session.execute("ALTER TABLE t ADD COLUMN val INTEGER DEFAULT 99");
    session.execute("UPDATE t SET val = 50 WHERE id = 1");

    const [result] = session.execute("SELECT val FROM t");
  ...
  it("DELETE works on rows with default-filled column", async () => {
    ...
    session.execute("ALTER TABLE t ADD COLUMN val INTEGER DEFAULT 99");
    session.execute("DELETE FROM t WHERE val = 99");
  ...
  it("ADD COLUMN defaults persist across reopen", async () => {
    ...
    s.execute("ALTER TABLE t ADD COLUMN val INTEGER DEFAULT 99");
    s.close();
    e.close();
...
describe("CREATE INDEX — __pk_ prefix", () => {
  it("rejects index name with __pk_ prefix", async () => {
    ...
    expect(() => session.execute("CREATE INDEX __pk_t ON t (name)")).toThrow(/__pk_/);
  });
});
...
describe("ALTER TABLE DROP COLUMN — index reference", () => {
  it("rejects DROP COLUMN when column is indexed", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, name TEXT, age INTEGER)");
    session.execute("CREATE INDEX idx_name ON t (name)");

    expect(() => session.execute("ALTER TABLE t DROP COLUMN name")).toThrow(/referenced by index/);
  });
...
describe("SUBSTR — negative start", () => {
  it("SUBSTR with negative start returns from end", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, s TEXT)");
    session.execute("INSERT INTO t VALUES (1, 'hello')");

    const [r1] = session.execute("SELECT SUBSTR(s, -1) AS v FROM t");
    expect(r1.rows![0].v).toBe("o");

    const [r2] = session.execute("SELECT SUBSTR(s, -2, 2) AS v FROM t");
...
describe("GROUP BY + aggregates", () => {
  it("GROUP BY with COUNT(*)", async () => {
    ...
    session.execute("INSERT INTO products VALUES (1, 'A', 10.0, 'Books')");
    session.execute("INSERT INTO products VALUES (2, 'B', 20.0, 'Books')");
    session.execute("INSERT INTO products VALUES (3, 'C', 30.0, 'Toys')");
  ...
  it("aggregate without GROUP BY", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, val INTEGER)");
  ...
  it("ORDER BY + LIMIT returns correct top-K rows", async () => {
    await createEngine();
    session.execute("CREATE TABLE scores (id INTEGER, val INTEGER)");
    session.execute("INSERT INTO scores VALUES (1, 50)");
    session.execute("INSERT INTO scores VALUES (2, 90)");
    session.execute("INSERT INTO scores VALUES (3, 10)");
    session.execute("INSERT INTO scores VALUES (4, 80)");
    session.execute("INSERT INTO scores VALUES (5, 70)");

    const [desc] = session.execute(
...
describe("parameterized queries", () => {
  it("execute() with $1 in INSERT", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, name TEXT)");
    session.execute("INSERT INTO t (id, name) VALUES ($1, $2)", [1, "alice"]);
  ...
  it("execute() with $1 in WHERE", async () => {
    ...
    session.execute("INSERT INTO t VALUES (2, 'bob')");

    const [result] = session.execute("SELECT * FROM t WHERE id = $1", [1]);
    expect(result.rows).toHaveLength(1);
  ...
  it("prepare() + run() executes multiple times with different params", async () => {
    ...
    const [result] = session.execute("SELECT * FROM t ORDER BY id");
    expect(result.rows).toHaveLength(3);
    expect(result.rows![0]).toEqual({ id: 1, name: "alice" });
  ...
  it("prepare() SELECT with $1 in WHERE returns correct row", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, val INTEGER)");
    session.execute("INSERT INTO t VALUES (1, 100)");
    session.execute("INSERT INTO t VALUES (2, 200)");
    session.execute("INSERT INTO t VALUES (3, 300)");
  ...
  it("UPDATE with parameterized SET and WHERE", async () => {
    ...
    session.execute("UPDATE t SET name = $1 WHERE id = $2", ["new", 1]);
  ...
  it("DELETE with parameterized WHERE", async () => {
    ...
    session.execute("DELETE FROM t WHERE id = $1", [1]);
...
describe("JSON type", () => {
  it("INSERT and SELECT with dot path access", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, data JSON)");
    session.execute(`INSERT INTO t (id, data) VALUES (1, '{"name": "Alice", "age": 30}')`);

    const [result] = session.execute("SELECT id, data.name, data.age FROM t");
    expect(result.rows).toHaveLength(1);
  ...
  it("WHERE filter with JSON path range comparison", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, data JSON)");
    session.execute(`INSERT INTO t (id, data) VALUES (1, '{"age": 30}')`);
    session.execute(`INSERT INTO t (id, data) VALUES (2, '{"age": 25}')`);

    const [result] = session.execute("SELECT id FROM t WHERE data.age > 20 AND data.age < 30");
  ...
  it("WHERE with single JSON path less-than", async () => {
    ...
    const [r1] = session.execute("SELECT id FROM t WHERE data.age < 30");
    expect(r1.rows!.map(r => r.id)).toEqual([2]);

    const [r2] = session.execute("SELECT id FROM t WHERE data.age > 20");
  ...
  it("DELETE with multi-expression filter on JSON path", async () => {
    ...
    session.execute(`INSERT INTO t (id, data) VALUES (1, '{"age": 30}')`);
    session.execute(`INSERT INTO t (id, data) VALUES (2, '{"age": 25}')`);
    session.execute(`INSERT INTO t (id, data) VALUES (3, '{"age": 15}')`);

    session.execute("DELETE FROM t WHERE data.age > 20 AND data.age < 30");

    const [result] = session.execute("SELECT id FROM t ORDER BY id");
  ...
  it("UPDATE with multi-expression filter on JSON path", async () => {
    ...
    session.execute(`UPDATE t SET data = '{"age": 99}' WHERE data.age > 20 AND data.age < 30`);

    const [result] = session.execute("SELECT id, data.age FROM t ORDER BY id");
    expect(result.rows![0]).toMatchObject({ id: 1 });
  ...
  it("CONCAT with JSON column produces JSON string", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, data JSON)");
    session.execute(`INSERT INTO t (id, data) VALUES (1, '{"name": "Alice"}')`);

    const [result] = session.execute("SELECT data.name || '!' AS greeting FROM t");
    expect(result.rows![0].greeting).toBe("Alice!");
  });

  it("CAST number/boolean to JSON preserves value", async () => {
    await createEngine();
    const [r1] = session.execute("SELECT CAST(42 AS JSON) AS v");
    expect(r1.rows![0].v).toBe(42);

    const [r2] = session.execute("SELECT CAST(TRUE AS JSON) AS v");
...
describe("BLOB type", () => {
  it("INSERT and SELECT blob literal", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, data BLOB)");
    session.execute("INSERT INTO t (id, data) VALUES (1, x'DEADBEEF')");

    const [result] = session.execute("SELECT id, data FROM t");
    expect(result.rows).toHaveLength(1);
  ...
  it("CAST blob to TEXT returns hex", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, data BLOB)");
    session.execute("INSERT INTO t VALUES (1, x'CAFE')");

    const [result] = session.execute("SELECT CAST(data AS TEXT) AS hex FROM t");
    expect(result.rows![0].hex).toBe("CAFE");
  });

  it("CAST text to BLOB", async () => {
    await createEngine();
    const [result] = session.execute("SELECT CAST('FF00' AS BLOB) AS v");
  ...
  it("blob parameter support", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, data BLOB)");
    session.execute("INSERT INTO t VALUES (1, $1)", [new Uint8Array([0xAA, 0xBB])]);

    const [result] = session.execute("SELECT data FROM t");
  ...
  it("blob comparison in WHERE", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, data BLOB)");
    session.execute("INSERT INTO t VALUES (1, x'AA')");
    session.execute("INSERT INTO t VALUES (2, x'BB')");

    const [result] = session.execute("SELECT id FROM t WHERE data = x'BB'");
    expect(result.rows!.map(r => r.id)).toEqual([2]);
  });
  ...
  it("ORDER BY blob column", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, data BLOB)");
    session.execute("INSERT INTO t VALUES (1, x'FF')");
    session.execute("INSERT INTO t VALUES (2, x'00')");
    session.execute("INSERT INTO t VALUES (3, x'AA')");

    const [result] = session.execute("SELECT id FROM t ORDER BY data");
  ...
  it("UPDATE blob column", async () => {
    ...
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, data BLOB)");
    session.execute("INSERT INTO t VALUES (1, x'AA')");
    session.execute("UPDATE t SET data = x'BB' WHERE id = 1");
  ...
  it("NULL blob", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, data BLOB)");
    session.execute("INSERT INTO t VALUES (1, NULL)");

    const [result] = session.execute("SELECT data FROM t WHERE id = 1");
