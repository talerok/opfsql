...
describe("EXPLAIN", () => {
  ...
  it("shows HashJoin for JOIN", async () => {
    await createEngine();
    session.execute("CREATE TABLE a (id INTEGER PRIMARY KEY)");
    session.execute("CREATE TABLE b (id INTEGER PRIMARY KEY, a_id INTEGER)");
    const plan = explain(
  ...
  it("shows IndexScan when index exists", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, val INTEGER)");
    session.execute("CREATE INDEX idx_val ON t (val)");
  ...
  it("result type is rows with plan column", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY)");
    const [result] = session.execute("EXPLAIN SELECT * FROM t");
    expect(result.type).toBe("rows");
    expect(result.rows).toHaveLength(1);
  ...
  it("ORDER BY via index returns correct results", async () => {
    ...
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, val INTEGER)");
    session.execute("CREATE INDEX idx_val ON t (val)");
    session.execute("INSERT INTO t VALUES (1, 30), (2, 10), (3, 20)");
    const [result] = session.execute("SELECT val FROM t ORDER BY val ASC");
  ...
  it("ORDER BY via index with LIMIT returns correct results", async () => {
    ...
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, val INTEGER)");
    session.execute("CREATE INDEX idx_val ON t (val)");
    session.execute("INSERT INTO t VALUES (1, 30), (2, 10), (3, 20), (4, 5)");
    const [result] = session.execute("SELECT val FROM t ORDER BY val ASC LIMIT 2");
  ...
  it("MIN via index returns correct result", async () => {
    ...
    session.execute("CREATE INDEX idx_val ON t (val)");
    session.execute("INSERT INTO t VALUES (1, 30), (2, 10), (3, 20)");
    const [result] = session.execute("SELECT MIN(val) AS m FROM t");
  ...
  it("MAX via index returns correct result", async () => {
    ...
    session.execute("CREATE INDEX idx_val ON t (val)");
    session.execute("INSERT INTO t VALUES (1, 30), (2, 10), (3, 20)");
    const [result] = session.execute("SELECT MAX(val) AS m FROM t");
  ...
  it("shows IndexUnionScan for OR with two indexed columns", async () => {
    await createEngine();
    session.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, a INTEGER, b INTEGER)");
    session.execute("CREATE INDEX idx_a ON t (a)");
    session.execute("CREATE INDEX idx_b ON t (b)");
  ...
  it("OR / Index Union returns correct results", async () => {
    ...
    session.execute("CREATE INDEX idx_a ON t (a)");
    session.execute("CREATE INDEX idx_b ON t (b)");
    session.execute("INSERT INTO t VALUES (1, 10, 100), (2, 20, 200), (3, 10, 200), (4, 30, 100)");
    const [result] = session.execute("SELECT id FROM t WHERE a = 10 OR b = 200 ORDER BY id");
  ...
  it("OR / Index Union deduplicates rows matching both branches", async () => {
    ...
    session.execute("CREATE INDEX idx_a ON t (a)");
    session.execute("CREATE INDEX idx_b ON t (b)");
    session.execute("INSERT INTO t VALUES (1, 5, 5)");
    const [result] = session.execute("SELECT id FROM t WHERE a = 5 OR b = 5");
    expect(result.rows!).toHaveLength(1);
