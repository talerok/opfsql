import type { CatalogData, ICatalog, IStorage, IndexDef, TableSchema } from './types.js';
...
export async function initCatalog(storage: IStorage): Promise<Catalog> {
  const data = await storage.get<CatalogData>(CATALOG_KEY);
